...
def check_group_status(group_name):
    ...
    try:
        response = subprocess.run(
            ["curl", "-X", "GET", f"http://{ip}:5050/list"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
...
def atualizar_midias():
    ...
    dispositivos = grupo.get('dispositivos', [])
    if not dispositivos:
        return jsonify({"status": "success", "media": {"active": [], "inactive": []}}), 200
...
def send_media_stream():
    ...
    for dispositivo in dispositivos:
        ...
        if ip:
            command = [
                "curl", "-X", "POST",
                "-F", f"file=@C:/PixelPlay/videos/{file_name}",
                "-F", f"start_time={start_time}",
                "-F", f"end_time={end_time}",
                f"http://{ip}:5050/upload"
            ]
            try:
